...
std::string RegisterManager::acquire_spillable_fp_temp_reg(NewCodeGenerator& code_gen) {
    ...
    for (const auto& reg : FP_VARIABLE_REGS) {
        if (registers[reg].status == FREE) {
            ...
            registers[reg].bound_to = ""; // Not bound to a named variable
            return reg;
        }
    }
    ...
    if (!fp_variable_reg_lru_order_.empty()) {
        ...
        // Spill to stack (assume current_frame_manager_ is accessible via code_gen)
        int offset = code_gen.get_current_frame_manager()->get_spill_offset(victim_var);
    ...
    throw std::runtime_error("No available spillable FP temp registers!");
}